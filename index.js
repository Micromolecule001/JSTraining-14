...
function isLeapYear(year) {
  ...
  return { Year: year, isLeap: status}
...
function addDays(date, days) {
  ...
  if (date !== Date) {
    if(typeof days === 'number') {
      ...
      return { ISODate, addedDays: days, newISODate };
...
function getDayOfWeek(date) {
  if (date !== Date) { 
    ...
    return {inputDate: isoDate, dayOfWeek: daysOfWeek[date.getDay()]}
  } else { return "Помилка: вхідне значення має бути об'єктом Date"; }
...
function getDaysInMonth(date) {
  if (date !== Date) { 
    ...
    return {inputDate: isoDate, daysInMounth: lastDay.getDate() }
  } else { return "Помилка: вхідне значення має бути об'єктом Date"; }
  // Перевірка, чи є вхідне значення об'єктом Date,це можно зробити перевіривши чи є date.getTime по типу функція .
  // Якщо date не є об'єктом Date, повертаємо рядок
  // "Помилка: вхідне значення має бути об'єктом Date"
...
function getFormattedTime(date) {
  if (date !== Date) { 
    ...
    return {inputDate: isoDate, formatedTime: `${hours.toString().padStart(2, "0")}:${minutes.toString().padStart(2, "0")}:${seconds.toString().padStart(2, "0")}`};
...
function getDaysDifference(startDate, endDate) {
  if (startDate !== Date || endDate !== Date) { 
    ...
    return {startDate: startDate.toISOString(), endDate: endDate.toISOString(), daysDifferens: days}  
  } else { return "Помилка: вхідне значення має бути об'єктом Date"; }
  // Перевірка, чи є вхідні значення об'єктами Date.
